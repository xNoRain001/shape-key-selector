...
def rename_shape_keys (key_blocks, categories, shape_keys, new_names, shape_key):
  def check ():
    passing = False

    for key_block in key_blocks:
      if key_block.name == 'shape_key_selector':
        passing = True
        break
  ...
  def foo (new_name, merged_shape_key_names_with_category, separate_mark):
  ...
  def add_done (shape_key):
    get_object().shape_key_add()
    done = get_shape_keys(shape_key)[-1]
    done.name = "shape_key_selector"

  passing = check()

  if passing:
    for key_block in key_blocks:
      foo(key_block.name, merged_shape_key_names_with_category, separate_mark)
  else:
    for key_block in key_blocks:
      ...
      if old_name in shape_keys:
        ...
        foo(new_name, merged_shape_key_names_with_category, separate_mark)
...
def gen_shape_key_selector (image_dir, overwrite, categories, self, shape_key):
  ...
  (
    ...
    merged_shape_key_names_with_category, 
    separate_mark
  ) = rename_shape_keys(key_blocks, categories, shape_keys, shape_key_names, shape_key)
